export const CREATE_BOOKING_REQUEST = 'CREATE_BOOKING_REQUEST';
export const CREATE_BOOKING_SUCCESS = 'CREATE_BOOKING_SUCCESS';
export const CREATE_BOOKING_FAILURE = 'CREATE_BOOKING_FAILURE';

export const GET_USER_BOOKINGS_REQUEST = 'GET_USER_BOOKINGS_REQUEST';
export const GET_USER_BOOKINGS_SUCCESS = 'GET_USER_BOOKINGS_SUCCESS';
export const GET_USER_BOOKINGS_FAILURE = 'GET_USER_BOOKINGS_FAILURE';

export const GET_COMPANY_BOOKINGS_REQUEST = 'GET_COMPANY_BOOKINGS_REQUEST';
export const GET_COMPANY_BOOKINGS_SUCCESS = 'GET_COMPANY_BOOKINGS_SUCCESS';
export const GET_COMPANY_BOOKINGS_FAILURE = 'GET_COMPANY_BOOKINGS_FAILURE';
